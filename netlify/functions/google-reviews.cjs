...
const Reviews = () => {
  ...
  const [hasFetched, setHasFetched] = useState(false); // NEW: Prevent multiple calls
  ...
  // Fetch Google reviews ONLY ONCE
  useEffect(() => {
    if (hasFetched) return; // STOP if already fetched
    ...
    const fetchGoogleReviews = async () => {
      try {
        ...
        if (data.success && data.reviews && data.reviews.length > 0) {
          ...
          console.log('⚠️ Using fallback reviews - API response:', data);
        ...
        setReviews(fallbackReviews);
        setError(true);
        setHasFetched(true); // Still mark as fetched to prevent retry
      } finally {
        setLoading(false);
      }
    };

    // Only fetch if component is in view
    if (inView) {
      fetchGoogleReviews();
    }
  }, [inView, hasFetched, fallbackReviews]); // Dependencies include hasFetched
